...
public class Function
{
    ...
    public async Task FunctionHandler(S3Event input, ILambdaContext context)
    {
        foreach(var record in input.Records)
        {
            ...
            var itemUrl = "https://" + record.S3.Bucket.Name + ".s3.amazonaws.com/" + record.S3.Object.Key;
            ImageDetails imageDetails = new ImageDetails(itemUrl, tags);
            var response = await DbHelper.insertItem(imageDetails);
            if (response)
            {
                Console.WriteLine("Image Metadata added to DynamoDB table");
            }
            else
            {
                Console.WriteLine("Image Metadata to DynamoDB table is unscuccessful!");
            }
